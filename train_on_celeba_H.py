...
def config():
    ...
    recon_type = 'MSE'
    activation = 'sigmoid'
    logdir = f'runs/CelebA-VAE_H-z_{z_dim}-{recon_type}-{activation}-beta={beta}'+ datetime.now().strftime('%y%m%d-%H%M%S')
    ex.observers.append(FileStorageObserver.create(logdir)) # saving source code
    
@ex.automain
def train(logdir, lr, epoches, beta, num_workers, image_size, z_dim, recon_type, warm_up, activation,device):   
