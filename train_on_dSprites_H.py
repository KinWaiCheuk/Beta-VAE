...
import os
from dataset import dSprites

from sacred import Experiment
from sacred.commands import print_config
from sacred.observers import FileStorageObserver

ex = Experiment('dSprites-VAE_H-z_10')
...
def train(logdir, lr, epoches, beta, num_workers, image_size, z_dim, recon_type, activation, warm_up, device):  
    ...
    dataset = dSprites('./data')
    train_set, test_set = torch.utils.data.random_split(dataset, [len(dataset)-5000,5000]) 
    train_loader = DataLoader(train_set,batch_size=256,shuffle=True,num_workers=16,
                            pin_memory=False,drop_last=True)
    test_loader = DataLoader(test_set,batch_size=32,shuffle=False,num_workers=16,
                            pin_memory=False,drop_last=True)
    
    x, _ = next(iter(dataset))
    net = BetaVAE_H(input_shape=x.shape,nc=1, z_dim=10, padding=0, activation=act_fn)
    net = net.to(device)
    ...
    for e in range(1,epoches+1):
        ...
        for idx, (x,_) in enumerate(train_loader):       
            x = x.to(device)
            x_recon, mu, logvar = net(x.unsqueeze(1))
            
            if e-1 < warm_up:
                recon_loss = reconstruction_loss(x.unsqueeze(1), x_recon, recon_type)
        ...
        if e%2==0:
            fig_recon = get_fig(x_recon.squeeze(1))
