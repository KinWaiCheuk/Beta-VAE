...
def config():
    ...
    recon_type = 'MSE'
    activation = 'sigmoid'
    logdir = f'runs/Chair-VAEv2_H-z_{z_dim}-{recon_type}-{activation}-beat={beta}-'+ datetime.now().strftime('%y%m%d-%H%M%S')
    ex.observers.append(FileStorageObserver.create(logdir)) # saving source code
    
    
@ex.automain
def train(logdir, lr, epoches, beta, num_workers, image_size, z_dim, recon_type, activation, device):   
    ...
    dataset = Chairs('./data', transform=Compose([Resize((image_size, image_size)),ToTensor()]))
    torch.manual_seed(0)
    ...
    for e in range(1,epoches+1):
        ...
        if e==1: # Showing the original transcription and spectrograms
            fig_x = get_fig(x.permute(0,2,3,1))
            writer.add_figure('images/Original', fig_x , e)
        if e%2==0:
            fig_recon = get_fig(x_recon.permute(0,2,3,1))
